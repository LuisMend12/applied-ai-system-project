...
def ai_assistant_section(songs, profile):
    ...
    if client is None:
        st.info(
            "Set the `ANTHROPIC_API_KEY` environment variable (or add it to a `.env` file) "
    ...
    if st.button("Get Recommendation", type="primary") and query.strip():
        ...
        mood = result.get("mood_label", "Mixed")
        ...
        if recs:
            ...
            song_map = {s["title"].lower(): s for s in songs}
            for title in recs:
                song = song_map.get(title.lower())
                if song:
                    tags = ", ".join(str(t) for t in song.get("tags", []))
                    st.success(
                        f"**{song['title']}** by {song['artist']} — "
                        f"genre: {song['genre']}, energy: {song['energy']}/10 · [{tags}]"
                    )
        else:
...
def main():
    ...
    main_tabs = st.tabs(["Playlists", "AI Assistant", "Stats & History"])
    ...
    with main_tabs[1]:
        ai_assistant_section(songs, profile)
