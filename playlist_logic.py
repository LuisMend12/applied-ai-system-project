...
def normalize_artist(artist: str) -> str:
    """Normalize an artist name — preserve case for display."""
    if not artist:
        return ""
    return artist.strip()
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    total = len(all_songs)  # fixed: was len(hype), causing ratio to always be 1.0
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        total_energy = sum(song.get("energy", 0) for song in all_songs)  # fixed: was only summing hype
...
def search_songs(
    ...
    for song in songs:
        ...
        if value and q in value:  # fixed: was `value in q` (backwards substring check)
...
def random_choice_or_none(songs: List[Song]) -> Optional[Song]:
    ...
    if not songs:  # fixed: random.choice crashes on empty list
        return None
